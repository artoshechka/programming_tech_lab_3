...
///     Изменение @ref factories_ и счётчика @ref nextTypeId_ не синхронизировано.
///   * Разрешение (GetObject<>) безопасно для чтения ТОЛЬКО при условии, что вся регистрация
///     уже завершена ДО старта дополнительных потоков. После этого @ref factories_ выступает
///     как фактически иммутабельный словарь, чтение из std::map с одной и той же ключевой
///     схемой без модификации — безопасно.
///   * Параллельная регистрация + резолв из разных потоков приведёт к гонке и/или порче map.
///
/// Типичный сценарий использования (см. gui/src/app_compositor.cpp): создать контейнер на стеке
/// в композиционном корне, выполнить все RegisterXxx, затем выполнить все GetObject, после
/// чего контейнер можно уничтожить — в фабриках уже захвачены все необходимые shared_ptr.
/// Запускать рабочие потоки только ПОСЛЕ завершения этой фазы.
class IOCContainer
{
