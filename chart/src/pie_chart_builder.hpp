...
class PieChartBuilder : public IChartBuilder
{
   public:
    ...
    /// @details Если включён флаг агрегации и число точек превышает порог,
    ///          данные предварительно группируются по префиксу ключа и усредняются.
    ///          Отображаются до 10 крупнейших срезов; остаток объединяется
    ///          в сектор "Other". Метки выводятся снаружи.
