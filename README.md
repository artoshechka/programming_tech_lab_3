...
#### Case №9
Проверка переключения агрегации (круговая диаграмма)
* Входные параметры: построена круговая диаграмма (Pie) по ряду с большим числом точек, например `test_data/SERIES_10K.json`
    * Шаг 1 — переключить флаг агрегации
* Результат: при включённой агрегации точки группируются (крупнейшие срезы + «Other»); при выключенной — показываются все срезы без агрегации
